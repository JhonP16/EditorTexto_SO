...
typedef struct {
    ...
    const char *category;    /* Categoría de ordenación: "datos", "memoria", "monitoreo", "utilidades", "aplicaciones" */
 ...
 * Categoría añadida por el equipo para integrar el editor de texto.
 * Agrupa programas externos interactivos a los que el shell cede la terminal. */
int cmd_editor(int argc, char **argv);     /* Syscalls: access, tcgetattr, fork, execv, waitpid, tcsetattr */
int cmd_editor_cmd(int argc, char **argv); /* Syscalls: pipe, fork, dup2, execv, write, close, waitpid */
