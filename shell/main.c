...
Command commands[] = {
    ...
    /* --- Categoría: Aplicaciones (añadida por el equipo para el editor) --- */
    {
        "editor", "aplicaciones",
        "editor [-c] [archivo]   (-c = intérprete de línea del editor)",
        "Lanza el editor cediéndole la terminal; reenvía las banderas tal cual.",
        "access(2), tcgetattr(3), fork(2), execv(2), waitpid(2), tcsetattr(3)",
        cmd_editor
    },
    {
        "editor_cmd", "aplicaciones",
        "editor_cmd <archivo> \"<comandos separados por ;>\"",
        "Ejecuta órdenes del editor sin interacción, por una tubería.",
        "pipe(2), fork(2), dup2(2), execv(2), write(2), close(2), waitpid(2)",
        cmd_editor_cmd
    },
...
void print_help(const char *arg) {
    if (arg == NULL) {
        ...
        printf("  " COLOR_CATEGORY "aplicaciones" COLOR_RESET " - Programas externos interactivos que toman la terminal (editor)\n\n");
        printf("Uso general:\n");
